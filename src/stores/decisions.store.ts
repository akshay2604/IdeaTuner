...
export const useDecisionsStore = create<DecisionsState>((set, get) => ({
  ...
  fetchDecisions: async () => {
    set({ isLoading: true })
    try {
      const decisions = await commands.listDecisions()
  ...
  fetchDecision: async (id: string) => {
    set({ isLoading: true })
    try {
      const decision = await commands.getDecision(id)
  ...
  createDecision: async (data?: Partial<Decision>) => {
    const decision = await commands.createDecision(data || {})
    const { decisions } = get()
  ...
  deleteDecision: async (id: string) => {
    await commands.deleteDecision(id)
  ...
  searchDecisions: async (query: string) => {
    ...
    try {
      const decisions = query
        ? await commands.searchDecisions(query)
        : await commands.listDecisions()
      set({ decisions, isLoading: false })
    } catch (error) {
  ...
  refreshCurrentDecision: async () => {
    const { currentDecision } = get()
    if (currentDecision) {
      const updated = await commands.getDecision(currentDecision.id)
      const decisions = await commands.listDecisions()
